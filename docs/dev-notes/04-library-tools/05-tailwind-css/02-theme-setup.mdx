...
- To prevent FOUC (Flash Of Unstyled Content) on page load, the class or data attribute on the `<html>` element needs to be set by:
  ...
  - An inline script in the `<head>` when user's preference is stored in `localStorage`.
    - This method prefers data attribute over class attribute. This is because when React defines a className on the html tag it'll replace the `.dark` class name that was added by the script defined in the `<head>` tag. (🚧 TODO: I'm confused by this. does this mean the content will flash dark mode, then wrongly convert back to light?)
...
🚧 TODO: perhaps an example of the above with real values makes sense?
